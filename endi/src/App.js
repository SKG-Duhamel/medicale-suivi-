...
import ReinitialiserPWD from './components/patient/ReinitialiserPWD';
import RegisterPatient from './components/patient/RegisterPatient';
import BarreNavigation from './components/patient/BarreNavigation';
...
function App() {
  return (
    <Routes>
      ...
      <Route path="/ReinitialiserPWD" element={<ReinitialiserPWD />} />
          

      <Route path="/patient" element={<BarreNavigation />}>
        {/* Routes enfants - chemins RELATIFS (sans / au début) */}
        <Route index element={<DashboardPatient />} />  {/* Page par défaut quand on va sur /patient */}
        <Route path="dashboard" element={<DashboardPatient />} />
        {/* Consultation avec ses routes enfants */}
